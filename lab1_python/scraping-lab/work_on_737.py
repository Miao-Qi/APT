...
def data_save(Option_type, tag_type, class_type, data_tag):
   element_counter = 0
   contract_dict["Type"] = Option_type
...
data_save(Option_type = "C", tag_type = "td", class_type = "yfnc_h", data_tag = c_data_tag)
data_save(Option_type = "C", tag_type = "td", class_type = "yfnc_tabledata1", data_tag = c_data_tag)
data_save(Option_type = "P", tag_type = "td", class_type = "yfnc_h", data_tag = p_data_tag)
data_save(Option_type = "P", tag_type = "td", class_type = "yfnc_tabledata1", data_tag = p_data_tag)   
...
all_sets_list = data_dict["optionQuotes"]
sorted_list = sorted(all_sets_list, key=lambda k: k['Strike'])
sorted_list = sorted(all_sets_list, key=lambda k: k['Type'])
sorted_list = sorted(all_sets_list, key = MyKey, reverse = True)
#sorted_list = sorted(all_sets_list, key = lambda user: (user["Open"]), reverse = True)
data_dict["optionQuotes"] = sorted_list  	 


# dump to json format
json.dump(data_dict, fp=dump_file, separators=(',', '[ ','{ '), sort_keys=True)
